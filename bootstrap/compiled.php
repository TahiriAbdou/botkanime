...
class Application extends Container implements HttpKernelInterface, TerminableInterface, ResponsePreparerInterface
{
    ...
    {
        return 'D:\\Users\\Abdou\\Dropbox\\WEBROOT\\botkanime\\laravel\\vendor\\laravel\\framework\\src\\Illuminate\\Foundation'.'/start.php';
    }
...
class Request
{
    ...
    protected static $trustedHeaders = [self::HEADER_CLIENT_IP => 'X_FORWARDED_FOR', self::HEADER_CLIENT_HOST => 'X_FORWARDED_HOST', self::HEADER_CLIENT_PROTO => 'X_FORWARDED_PROTO', self::HEADER_CLIENT_PORT => 'X_FORWARDED_PORT'];
...
class FileBag extends ParameterBag
{
    ...
    protected function fixPhpFilesArray($data)
    {
        ...
        foreach (array_keys($data['name']) as $key) {
            $files[$key] = $this->fixPhpFilesArray(['error' => $data['error'][$key], 'name' => $data['name'][$key], 'type' => $data['type'][$key], 'tmp_name' => $data['tmp_name'][$key], 'size' => $data['size'][$key]]);
        }
...
class ExceptionHandler
{
    ...
    public function handle(\Exception $exception)
    {
        ...
        ob_start([$this, 'catchOutput']);
        $this->failSafeHandle($exception);
        while (null === $this->caughtBuffer && ob_end_flush()) {
        }
        if (isset($this->caughtBuffer[0])) {
            ob_start([$this, 'cleanOutput']);
                    ...
                    foreach ($e['trace'] as $trace) {
                        ...
                        if (isset($trace['file']) && isset($trace['line'])) {
                            if ($linkFormat = ini_get('xdebug.file_link_format')) {
                                $link = str_replace(['%f', '%l'], [$trace['file'], $trace['line']], $linkFormat);
...
class ErrorHandler
{
    ...
    public function handle($level, $message, $file = 'unknown', $line = 0, $context = [])
    {
        if ($level & (E_USER_DEPRECATED | E_DEPRECATED)) {
            if (isset(self::$loggers['deprecation'])) {
                if (self::$stackedErrorLevels) {
                    ...
                    self::$loggers['deprecation']->warning($message, ['type' => self::TYPE_DEPRECATION, 'stack' => $stack]);
                }
        ...
        if (isset(self::$loggers['scream']) && !(error_reporting() & $level)) {
            if (self::$stackedErrorLevels) {
                ...
                self::$loggers['scream']->log($logLevel, $message, ['type' => $level, 'file' => $file, 'line' => $line, 'scream' => error_reporting()]);
            }
        }
    ...
    public function handleFatal()
    {
        ...
        if (isset(self::$loggers['emergency'])) {
            $fatal = ['type' => $error['type'], 'file' => $error['file'], 'line' => $error['line']];
...
class UrlGenerator
{
    ...
    protected $dontEncode = ['%2F' => '/', '%40' => '@', '%3A' => ':', '%3B' => ';', '%2C' => ',', '%3D' => '=', '%2B' => '+', '%21' => '!', '%2A' => '*', '%7C' => '|'];
...
class Logger implements LoggerInterface
{
    ...
    public function addRecord($level, $message, array $context = [])
    {
        ...
        $record = ['message' => (string) $message, 'context' => $context, 'level' => $level, 'level_name' => static::getLevelName($level), 'channel' => $this->name, 'datetime' => \DateTime::createFromFormat('U.u', sprintf('%.6F', microtime(true)), static::$timezone)->setTimezone(static::$timezone), 'extra' => []];
        $handlerKey = null;
...
class RotatingFileHandler extends StreamHandler
{
    ...
    {
        $fileInfo = pathinfo($this->filename);
        $glob = str_replace(['{filename}', '{date}'], [$fileInfo['filename'], '*'], $fileInfo['dirname'].'/'.$this->filenameFormat);
        if (!empty($fileInfo['extension'])) {
...
class SymfonyDisplayer implements ExceptionDisplayerInterface
{
    ...
    {
        if ($this->returnJson) {
            return new JsonResponse(['error' => $exception->getMessage(), 'file' => $exception->getFile(), 'line' => $exception->getLine()], 500);
        } else {
